...
void advance_level( CHAR_DATA *ch, bool hide )
{
    ...
    sprintf_s( buf, 4096, "the %s", title_table [ch->class] [ch->level] [ch->sex == SEX_FEMALE ? 1 : 0] );
    set_title( ch, buf );
...
void gain_exp( CHAR_DATA *ch, int gain )
{
	...
	ch->level += 1;
	sprintf_s(buf,4096,"%s gained level %d",ch->name,ch->level);
	log_string(buf);
	sprintf_s(buf,4096,"$N has attained level %d!",ch->level);
